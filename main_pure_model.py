...
if world.tensorboard:
    w: SummaryWriter = SummaryWriter(join(world.BOARD_PATH, time.strftime("%m-%d-%Hh%Mm%Ss-") + "-" + world.method + '-' + world.model_name + "-" + world.dataset))
else:
    w = None
    world.cprint("not enable tensorflowboard")

item_dict = {}
print(datapath + '/' + pklname)
...
try:
    ...
    for epoch in range(world.TRAIN_epochs):
        ...
        if epoch % 10 == 0 and epoch != 0:
            ...
            cprint("[test]")
            user_results, item_results = Procedure.Test(datapath, key_genre, dataset, Recmodel, epoch, w, world.config['multicore'])
            ...
            if update_flag:
                print('saving best model', weight_file)
        ...
        # train process
        # dataset, key_genre, item_dict, neg_prob, neg_ratio=5
        S = utils.UniformSample_original(datapath, dataset, key_genre, item_dict, neg_prob, neg_ratio=1)
        if epoch % 10 == 0:
        ...
        for batch_i, (batch_users, batch_pos, batch_neg, _) in enumerate(train_loader):
            cri = bpr.stageOne_pure(batch_users.to(world.device), batch_pos.to(world.device), batch_neg.to(world.device), datapath, pklname, key_genre, batch_pos)
            aver_loss += cri
            if world.tensorboard:
