...
# class hardNeg_Sample_python_failure():
#     def __init__(self, dataset, datapath, pklname, model, prob, key_genre):
#         self.rec_model = model
#         self.prob = prob
#         self.outer_optimizer = AdamOptimizer(self.prob)
...
#         self.train_pairs = []
#         self.user_neg_fairprob = {}
#         self.u_index = {}
#         self.uz_index = {}
#         self.ind = 0
#
#         for i in range(dataset.n_users):
#             self.user_neg_fairprob[i] = prob
#             self.u_index[i] = []
#             for j in self.allPos[i]:
#                 self.u_index[i].append(self.ind)
#                 self.ind += 1
#                 self.train_pairs.append((i, j))
#
#         self.zp_attribute = []
...
#             self.zp_len[tmp_z] = len(self.zp_index[tmp_z])
#
#         for u in self.u_index.keys():
#             self.uz_index[u] = {}
#             u_pos_index = self.u_index[u]
#             for tmp_z in self.z_item:
#                 z_index = self.zp_index[tmp_z]
#                 uz_index = [i for i in u_pos_index if i in z_index]
#                 self.uz_index[u][tmp_z] = uz_index
#         print('initialized finished')
#
#     def softmax_with_temperature(self, X, T = 0.2):
#         X = X / T
#         max_prob = np.max(X, axis=0)
#         X -= max_prob
#         exp_x = np.exp(X, X)
...
#         return prob
#
#     def cal_fair_prob(self, loss, user):
#         yhat_yzp = {}
#         for tmp_z in self.z_item:
#             zu_index = self.uz_index[user][tmp_z]
#             if len(zu_index) == 0:
#                 yhat_yzp[tmp_z] = 0
#             else:
#                 yhat_yzp[tmp_z] = float(torch.sum(loss[zu_index])) / len(zu_index)
#
#         yhat_yzp_items = list(yhat_yzp.values())
#         yhat_yzp_avg = sum(yhat_yzp_items) / len(yhat_yzp_items)
#
#         cur_grads = np.array([y - yhat_yzp_avg for y in yhat_yzp_items])
#         self.user_neg_fairprob[user] = self.outer_optimizer.step(params=self.user_neg_fairprob[user], grads=cur_grads)
#         self.user_neg_fairprob[user][-1] += 1 - sum(self.user_neg_fairprob[user])
#         return self.user_neg_fairprob[user]
#
#     def fair_aware_prob(self, ss_group_prob, candidate_items, key_genre):
#         candidate_items_categroy = np.array([key_genre.index(list(set(self.item_idd_category[i]).intersection(set(key_genre)))[0]) \
#                                     for i in candidate_items])
#
#         prob = np.zeros(candidate_items_categroy.shape[0], dtype=np.float32)
#         for k in range(len(key_genre)):
...
#         return prob
#
#     def sample_neg(self, dataset, datapath, key_genre, pklname, Recmodel):
#         dataset: BasicDataset
#         Recmodel = Recmodel.eval()
#         users = [i for i in range(dataset.n_users)]
#         items = [j for j in range(dataset.m_items)]
...
#         with torch.no_grad():
#             users_gpu = torch.Tensor(np.array(users)).long().cuda()
#             users1_gpu = torch.Tensor(np.array(self.train_pairs)[:, 0]).long().cuda()
#             pos_gpu = torch.Tensor(np.array(self.train_pairs)[:, 1]).long().cuda()
#             # 3845 * 2487
#             ratings = Recmodel.getUsersRating(users_gpu).cpu().numpy()
#             loss = Recmodel.get_pos_logit(users1_gpu, pos_gpu)
#
#             for i, user in enumerate(users):
#                 start = time()
#                 posForUser = allPos[user]
...
#                 # neg_sort_index = np.argsort(ratings[user][candidate_items])
#                 # neg_order = np.zeros(neg_scores.shape[0])
#                 #
#                 # for i in range(len(neg_sort_index)):
#                 #     neg_order[neg_sort_index[i]] = i
#                 # hard_aware_neg_prob = self.ranking_aware_prob(neg_order)
#                 hard_aware_neg_prob = self.softmax_with_temperature(X=neg_scores)
#                 fair_aware_neg_group_prob = self.cal_fair_prob(loss, user)
#                 fair_aware_neg_prob = self.fair_aware_prob(fair_aware_neg_group_prob, candidate_items, key_genre)
#
#                 assert len(hard_aware_neg_prob) == len(fair_aware_neg_prob)
#
#                 total_prob = 0.05 * np.array(hard_aware_neg_prob) + 0.95 * np.array(fair_aware_neg_prob)
#                 total_prob[-1] += 1 - sum(total_prob)
#
#                 for j in range(len(posForUser)):
#                     pos_index = j
